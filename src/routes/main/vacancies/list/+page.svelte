<script lang="ts">
	...
	async function getCollections() {
		const result = await api.from('vacancies').getList(query);
		return result;
	}

	async function save(data: NonNullable<Forms['save']>) {
		const result = await api.from('vacancies').save(data);
	...
	async function del(data: NonNullable<Forms['del']>) {
		...
		await api.from('vacancies').delete(ids);
		forms.del = undefined;
		refresh();
	}
	...
	{#snippet children(items)}
		<form class="mt-5 flex flex-col gap-5" onsubmit={() => save(items)}>
			{#each Object.values(items) as item}
				...
				<label class="label floating-label">
					<input type="file" bind:files={item.media} />
				</label>
			{/each}
			<div>
...
<div class="flex flex-wrap items-center gap-2 px-3">
	<h1 class="mt-1 ml-12 text-xl capitalize">Daftar Lowongan</h1>
</div>
