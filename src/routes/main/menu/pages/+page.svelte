<script lang="ts">
	...
	async function getCollections() {
		const result = await api.from('pages').getList(query);
		return result;
	}

	async function save(data: NonNullable<Forms['save']>) {
		const result = await api.from('pages').save(data);
	...
	async function del(data: NonNullable<Forms['del']>) {
		...
		await api.from('pages').delete(ids);
		forms.del = undefined;
		refresh();
	}
...
<div class="flex flex-wrap items-center gap-2 px-3">
	<h1 class="mt-1 ml-12 text-xl capitalize">Daftar Halaman</h1>
</div>
