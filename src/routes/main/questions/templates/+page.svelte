		...
		<form class="mt-5 flex flex-col gap-5" onsubmit={() => save(items)}>
			{#each Object.values(items) as item}
				...
				<label class=" floating-label">
					<span>Template Pertanyaan</span>
					<input
						type="text"
						class="input w-full"
						placeholder="Template Pertanyaan"
				...
				<label class=" floating-label">
					<span>Group</span>
					<input class="input w-full" placeholder="Deskripsi" bind:value={item.description} />
				</label>
				<Select
...
<div class="flex flex-wrap items-center gap-2 px-3">
	<h1 class="mt-1 ml-12 text-xl capitalize">Daftar Template Pertanyaan</h1>
</div>
